...
- **블로그명:** 이더
- **로고:** 이더.dev (점(.)에 액센트 색 적용)
- **운영자:** 이더 (KimYoungtae) — UE5 C++ 게임 프로그래머 / AI × 사이드프로젝트 빌더
...
```
Railway (단일 플랫폼)
├── ether-blog (Next.js 앱)      — 블로그 + API + 관리자
├── ether-db (PostgreSQL)         — LAMDiceBot DB와 같은 인스턴스 또는 별도
├── ether-umami (Umami)           — 통계 수집/대시보드
